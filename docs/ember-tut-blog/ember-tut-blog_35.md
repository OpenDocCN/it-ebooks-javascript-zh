# Ember.js 入门指南之三十一自定义包裹组件的 HTML 标签
...
页面重新加载之后查看源代码，可以看到`nav`标签中多了一个 CSS 类`urgent`，如果属性`urgent`的值为`false`，CSS 类`urgent`将不会渲染到`nav`标签上。
...
此时页面的 HTML 源码就有点不一样了。`renderedClassName`作为 CSS 类名被渲染到了`nav`标签上。
